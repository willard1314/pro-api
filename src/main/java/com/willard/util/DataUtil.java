...
public class DataUtil {
     ...
     */
    public static String gettNowDateString(){
    	SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");	
    	return df.format(new Date());
    }
    
    /**
     * 计算给出时间计算并格式化后的字符串。第二个参数表示给出时间的时间差。正数表示XX天以后，负数表示XX天以前。
     * eg: String sessionTimeoutLimitStr = DataUtil.calDatetimeString(new Date(),  -7);
     * 
     * @param date 时间格式
     * @param adjustSeconds 时间调整数，单位：天
     * @return 返回调整时间后的格式化时间字符串
     */
    public static String calDatetString(Date date, int adjustSeconds){
    	if(adjustSeconds!=0) date.setTime(date.getTime() + adjustSeconds*1000*60*60*24);
    	SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
    	return df.format(date);
    }
